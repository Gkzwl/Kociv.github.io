...
const timerDisplay = document.getElementById('timer'); // 计时器显示
const restartGameButton = document.getElementById('restart-game');
let selectedTiles = [];
let score = 0;
const boardSize = 8;
const totalPairs = (boardSize * boardSize) / 2;
let timeLeft = 5 * 60; // 10分钟转换为秒
let timer;
...
function startTimer() {
    timer = setInterval(() => {
        ...
        timerDisplay.textContent = `时间: ${minutes}:${seconds < 5 ? '0' : ''}${seconds}`;

        if (timeLeft <= 0) {
            clearInterval(timer);
            alert('时间到！游戏结束。');
            // 可以在这里添加游戏结束的逻辑，比如显示最终得分
            window.location.href = 'index.html'; // 或者跳转到结束页面
        }
    }, 1000);
}
